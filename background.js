...
async function getSessionCookie(origin, storeId) {
  try {
    ...
    let details = { url: cookieUrl, name: "sid" };
    if (storeId) {
    ...
    for (let domain of orderedDomains) {
      details.url = `https://${domain}`;
      let altCookie = await new Promise(resolve => {
        chrome.cookies.get(details, resolve);
      });
      if (altCookie && altCookie.value) return altCookie.value;
    }
    return null;
  } catch (error) {
